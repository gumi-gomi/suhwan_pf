          ...
          <ModalContent onClick={(e) => e.stopPropagation()}> {/* 모달 클릭 시 닫히지 않도록 */}
            ...
            <p className='date'>2024. 11</p>
                ...
                <p className='meaningDetail'>
                  ...
                  작업하면서 재미있었던 것은 실제 사용 가능한 기능을 구현하려다보니 화면만 구성할 때 보지 못했던 오류들을 만나고 새로운 기능을 사용해본 것 입니다.<br/>Firebase에서 Auth와 Database를 활용하여 각 회원마다 정보를 저장할 수 있도록 하며, 로그인 후 카카오페이 결제를 실행했을 때 로그인되어있는 회원의 정보와 상품페이지의 상품번호,이름을 활용하여 가격을 불러오고 결제 완료후 해당 상품의 다운로드 링크를 마이페이지에 추가되도록하는 작업도 하였습니다.<br/><br/> 또한 지금까지는 React로 작업한 작업물을 빌드하여 깃허브에 올려서 볼 수 있도록 했었는데 가비아에서 도메인을 구매하고 Firebase Hosting과 연결하여 해당 도메인으로 사이트에 접속할 수 있도록 하였습니다.<br/><br/> 이러한 과정을 거치며 페이지를 화면에 보이게하는것도 중요하지만, 서비스를 운영하기 위해서는 회원이 화면에서 하는 일련의 행동들에서 생기는 데이터를 저장하고 관리하는 것이 중요하다는 사실을 배웠습니다. 일단 서비스를 이용할 수 있도록 하는 필수적인 기능들만 사용했지만 사이트를 더 편하게 그리고 안정적으로 운영할 수 있도록 지속적으로 새로운 기능을 탐색하고 디자인을 수정하려고 합니다. <br/><br/>
                  <span style={{color:'rgba(256,0,0,1)',fontWeight:"700"}}>(현재 카카오페이 가맹점 심사가 늦어져서 실제 거래는 할 수 없고 회원가입 후 테스트결제로 확인할 수 있습니다.)</span>
          ...
          <ModalContent onClick={(e) => e.stopPropagation()}> {/* 모달 클릭 시 닫히지 않도록 */}
            ...
            <p className='date'>2024. 11</p>
                ...
                <p className='meaningDetail'>
                  ...
                  먼저 가상의 치과인 '바른이치과'의 홈페이지를 만들고 Firebase의 회원가입 기능을 넣은 뒤, 회원마다 마이페이지에서 문의사항을 남길 수 있도록 하였습니다. 또한 관리자페이지는 Admin계정으로 접속 할 수 있게 하였고, 관리자페이지에서 회원의 문의사항을 보고 답글을 달 수 있도록 하였습니다.
